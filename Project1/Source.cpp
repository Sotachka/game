...
void Enemy::Interact(Player& p, std::string cmd)  
{
    ...
    if(cmd == "attack") 
    {
        ...
        if (hp <= 0)
        {
            ...
            takenDamage = weapon->damage; //запоминаем урон наносимый врагом.
            if (p.armor != nullptr)
            {
                takenDamage -= p.armor->defense; //отнимаем от урона кол-во брони на игроке.
                if (takenDamage > 0)
                {
                    
                    p.hp -= takenDamage;//наносим итоговый урон игроку
                }
                else
                {
